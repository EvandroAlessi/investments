...
export class FoundsDistributionChartComponent implements OnInit, AfterContentInit {
  ...
  async bySelectedChanged(value: number): Promise<void> {
  ...
  private async buildChart(): Promise<void> {
    const expectationData = await this.buildChartData(true);
  ...
  private async buildChartData(isExpected: boolean): Promise<number[]> {
    const data: number[] = [];
